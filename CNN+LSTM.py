...
iters = 300
comb = list(product(n_CNN, n_LSTM, n_dense, filters, kernels, units,
                    dropouts, neurons))
...
for t in mercados:
    ...
    for c in range(iters):
        ...
        cnn_lstm_model = models.Sequential([
            layers.Input(shape=(horas[cont]+horas[cont+1], 150, 1))
        ])
        
        for j in range(0, comb[sel][0]):
            cnn_lstm_model.add(layers.Conv2D(filters=comb[sel][3], 
                                        kernel_size=(comb[sel][4], comb[sel][4]), 
                                        activation='relu', padding='same'))
            cnn_lstm_model.add(layers.MaxPooling2D(pool_size=(2, 2)))
            
        cnn_lstm_model.add(layers.Reshape(((horas[cont]+horas[cont+1])//(2**comb[sel][0]),
                                           150//(2**comb[sel][0]) * comb[sel][3])))
        
        if comb[sel][1] == 1:
            cnn_lstm_model.add(layers.LSTM(units=comb[sel][5], 
        ...
        for k in range(0, comb[sel][2]):
            cnn_lstm_model.add(layers.Dense(comb[sel][7], activation = "relu"))
            
        cnn_lstm_model.add(layers.Dense(horas[cont+1]*150)),
        cnn_lstm_model.add(layers.Reshape((horas[cont+1],150)))
        
        # Optimize
        optimizer = Nadam(learning_rate=0.001)
        cnn_lstm_model.compile(optimizer=optimizer, loss=custom_loss[cont])
        
        try:
            # Fit
            cnn_lstm_model.fit(train_input, train_output, epochs = 50, verbose = 0)
            
            # Predict
            predictions = cnn_lstm_model.predict(val_input, verbose = 0)
            ...
            if mean < best_mean:
                best_mean = mean
                best_n_CNN = comb[sel][0]
                best_n_LSTM = comb[sel][1]
                best_n_dense = comb[sel][2]
                best_filter = comb[sel][3]
                best_kernel = comb[sel][4]
                best_unit = comb[sel][5]
                best_dropout = comb[sel][6]
                best_neuron = comb[sel][7]
    ...
    bests = [best_mean, best_n_CNN, best_n_LSTM, best_n_dense, best_filter,
             best_kernel, best_unit, best_dropout, best_neuron]
    ...
    for j in range(0, best_n_CNN):
        cnn_lstm_model.add(layers.Conv2D(filters=best_filter, 
                                    kernel_size=(best_kernel, best_kernel), 
                                    activation='relu', padding='same'))
        cnn_lstm_model.add(layers.MaxPooling2D(pool_size=(2, 2)))
        
    cnn_lstm_model.add(layers.Reshape(((horas[cont]+horas[cont+1])//(2**best_n_CNN),
                                       150//(2**best_n_CNN) * best_filter)))
    
    if best_n_LSTM == 1:
        cnn_lstm_model.add(layers.LSTM(units=best_unit, 
                                    return_sequences=False, 
    ...
    for k in range(0, best_n_dense):
        cnn_lstm_model.add(layers.Dense(best_neuron, activation = "relu"))
    ...
    try:
        ...
        # Predict
        predictions = cnn_lstm_model.predict(test_input, verbose = 0)
