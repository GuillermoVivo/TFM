...
iters = 300
comb = list(product(epochs, n_layers_dense, n_layers_conv, filters, 
                    kernels, neurons))
...
for t in mercados:
    ...
    for c in range(iters):
        ...
        for j in range(0, comb[sel][2]):
            cnn_model.add(layers.Conv2D(filters=comb[sel][3], 
        ...
        for k in range(0, comb[sel][1]):
            cnn_model.add(layers.Dense(comb[sel][5], activation = 'relu'))
            
        cnn_model.add(layers.Dense(horas[cont+1] * 150, activation='relu'))
        cnn_model.add(layers.Reshape((horas[cont+1], 150)))
        
        # Compile and fit
        cnn_model.compile(optimizer='nadam', loss=custom_loss[cont])
        cnn_model.fit(train_input, train_output, epochs=comb[sel][0], verbose = 0)
        
        # Predict
        predictions = cnn_model.predict(val_input, verbose = 0)
        ...
        if mean < best_mean:
            ...
            best_n_conv = comb[sel][2]
            best_fil = comb[sel][3]
            best_kernel = comb[sel][4]
            best_neuron = comb[sel][5]
    ...
    bests = [best_mean, best_epoch, best_n_dense, best_n_conv, 
                  best_fil, best_kernel, best_neuron]
    ...
    for j in range(0, best_n_conv):
        cnn_model.add(layers.Conv2D(filters=best_fil, 
    ...
    for k in range(0, best_n_dense):
        cnn_model.add(layers.Dense(best_neuron, activation = 'relu'))
    ...
    # Compile and fit
    cnn_model.compile(optimizer='nadam', loss=custom_loss[cont])
    cnn_model.fit(train_input, train_output, epochs=best_epoch, verbose = 0)
    
    # Predict
    predictions = cnn_model.predict(test_input, verbose = 0)
